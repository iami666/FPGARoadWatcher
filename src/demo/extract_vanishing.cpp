...
int main(int argc, const char *argv[]) {
  ...
  LiteSignature signature = {
    .edge_threshold = 10,
    .theta_limit = 80,
    .hough_treshold = 20,
    .blur_size = 5,
    .mask_mid_y = 180, // height(= 360) / 2
    .mask_mid_value = 30,
    .mean_x = 0,
    .mean_y = 0,
    .display_extra = true,
  ...
  //const string & filename = "../solidWhiteRight.jpg";
  const string &filename = "../road.mp4";

  //rgb_img_t src = imread(, IMREAD_COLOR);
  rgb_img_t out;

  VideoCapture vidcap = VideoCapture();
  vidcap.open(filename);
  rgb_img_t src;

  //std::cout << "Width: " << src.cols << " height: " << src.rows << std::endl;
  //extract_vanishing(src, out, &signature);
  //std::cout << "Vanishing X: " << signature.mean_x << " Vanishing Y: " << signature.mean_y << std::endl;

  bool pause = false;
  int image_index = 0;
  while (true) {
    if (not pause) {
      if (not vidcap.read(src))
        break;
      extract_vanishing(src, out, &signature);
      std::cout << "Index: " << image_index << " Vanishing X: " << signature.mean_x << " Vanishing Y: "
                << signature.mean_y << std::endl;
      imshow("source", out);
      image_index++;
    }
    ...
    if (key == 32)
      ...
      signature.display_raw = not signature.display_raw;
    else if (key == 101) // E
      signature.display_extra = not signature.display_extra;
    else if (image_index == 3703) {
      pause = true;
    }
  }
  //imshow("source", out);
...
void draw_lines(Mat &img, cv::Vec3f (&lines)[TOP_LINES * 2]) {
  float a;
  char p;
  cv::Vec<unsigned char, 3> pixelh;       //pixel to add to hough image
  cv::Vec<unsigned char, 3> pixela;       //pixel to read image
  int rows = img.rows;
  int cols = img.cols;
